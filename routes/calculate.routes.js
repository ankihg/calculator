  ...
  router.route('/calculate')
    .post((req, res) => {
      ...
      if (!((req.body.operand1 || req.body.operand1 === 0) && (req.body.operand2 || req.body.operand2 === 0)))
        return res.status(400).json({msg:'insufficient operands; cannot calculate', err: new Error('insufficient operands')});

      req.body.operator = operators[req.body.operator];
      if (!req.body.operator)
        return res.status(400).json({msg:'insufficient operator; cannot calculate', err: new Error('insufficient operator')});
